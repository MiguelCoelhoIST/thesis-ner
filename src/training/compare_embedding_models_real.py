...
EVAL_PATH = Path("data/eval/roles_eval_real_candidates_v1.jsonl")
RESULTS_PATH = Path("results/experiment_18_real_encoder_comparison.txt")
...
def load_real_eval_jsonl(path):
    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            text = record["text"]
            start = record["entity_start"]
            end = record["entity_end"]
            entity = record["entity"]

            if text[start:end] != entity:
                raise ValueError(
                    f"Example {record['id']}: expected entity {entity!r}, "
                    f"found {text[start:end]!r}"
                )

            if "human_validated" in record and record["human_validated"] is not True:
                raise ValueError(f"Example {record['id']}: human_validated is not True")
...
def main():
    ...
    train_texts, train_labels = load_train_jsonl(TRAIN_PATH)
    eval_texts, eval_labels = load_real_eval_jsonl(EVAL_PATH)
    ...
    with RESULTS_PATH.open("w", encoding="utf-8") as f:
        f.write("Experiment 18 - Real STJ encoder comparison\n\n")
